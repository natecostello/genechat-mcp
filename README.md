...
> **Disk usage:** `genechat init --gnomad` downloads each gnomAD chromosome, annotates it, then deletes the file — peak disk usage is ~17 GB (one chromosome) rather than ~150 GB for all files at once. `genechat download --gnomad` downloads all files and keeps them for reuse across multiple genomes.
