...
class TestQueryGenes:
    def test_multiple_genes(self, mock_engine, mock_engines, test_db, test_config):
    ...
    def test_empty_input(self, mock_engine, mock_engines, test_db, test_config):
        fn = _setup_tool(mock_engines, test_db, test_config)
        result = fn(genes="")
        assert "Please provide" in result

    def test_too_many_genes(self, mock_engine, mock_engines, test_db, test_config):
        fn = _setup_tool(mock_engines, test_db, test_config)
        many = ",".join(f"GENE{i}" for i in range(21))
        result = fn(genes=many)
        assert "Too many" in result

    def test_no_notable_variants(self, mock_engine, mock_engines, test_db, test_config):
