...
class TestGwasZipCleanup:
    def test_zip_deleted_after_build(self, tmp_path, capsys):
        """Verify GWAS zip is deleted after successful DB build to free disk."""
        # Create a minimal valid GWAS catalog zip
        tsv_content = (
        ...
        with zipfile.ZipFile(str(zip_path), "w") as zf:
            zf.writestr("gwas-catalog.tsv", tsv_content)

        db_path = tmp_path / "gwas.db"
        build_gwas_db(zip_path=zip_path, db_path=db_path)

        # DB should exist, zip should be deleted
