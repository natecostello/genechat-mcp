...
class TestQueryClinvarGIAB:
    def test_pathogenic_returns_results(self, giab_engine, giab_db, giab_config):
    ...
    def test_drug_response_returns_results(self, giab_engine, giab_db, giab_config):
        """Drug response ClinVar query should return results if annotated."""
        fn = _get_tool(giab_engine, giab_db, giab_config)
        result = fn(significance="drug_response", max_results=10)
        assert isinstance(result, str)

    def test_clinvar_with_gene_filter(self, giab_engine, giab_db, giab_config):
    ...
    def test_benign_returns_results(self, giab_engine, giab_db, giab_config):
