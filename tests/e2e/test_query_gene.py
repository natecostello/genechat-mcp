...
class TestQueryGeneGIAB:
    def test_cyp2d6_finds_variants(self, giab_engine, giab_db, giab_config):
    ...
    def test_brca1_returns_variants(self, giab_engine, giab_db, giab_config):
        """BRCA1 region should have common variants."""
        fn = _get_tool(giab_engine, giab_db, giab_config)
        result = fn(gene="BRCA1", impact_filter="HIGH,MODERATE,LOW,MODIFIER")
        assert "BRCA1" in result

    def test_nonexistent_gene(self, giab_engine, giab_db, giab_config):
        """Unknown gene should return not found."""
        fn = _get_tool(giab_engine, giab_db, giab_config)
        result = fn(gene="FAKEGENE123")
        assert "not found" in result.lower()

    def test_impact_filter(self, giab_engine, giab_db, giab_config):
